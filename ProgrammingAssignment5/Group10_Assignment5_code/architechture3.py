...
def make_convnet():
    ...
    #conv block 4
    x = Conv2D(filters=64, kernel_size=(3,3), strides=1, padding="valid", activation='relu')(x)
    x = MaxPooling2D(pool_size=(3, 3), strides=2, padding="valid")(x)
    ...
    return Model(inputs=[image], outputs=[x], name="Architecture3")
...
def main():
    ...
    model.save(filepath="./ProgrammingAssignment5/models/architecture3.h5", overwrite=True, include_optimizer=True)
    with open("./ProgrammingAssignment5/logs/hist_arch3.pkl", mode="wb") as f:
