...
# # optimizer
# sgd = keras.optimizers.SGD(learning_rate=0.001, momentum=0, name="batch")
...
#                     batch_size=11385, epochs=100_000,
#                     callbacks=[earlystopping],
#                     verbose=0, shuffle=True,
#                     validation_data=[val_data, val_labels])

# # saving
# model_1.save(filepath="batch_model1.h5", overwrite=True, include_optimizer=True)

# with open('batch_history1.pkl', mode='wb') as f:
...
# # optimizer
# sgd = keras.optimizers.SGD(learning_rate=0.001, momentum=0, name="SGD")
# earlystopping = keras.callbacks.EarlyStopping(monitor='loss',
#                                               min_delta=1e-4,
#                                               patience=1,
#                                               verbose=1)
...
# # saving
# model_2.save(filepath="batch_model2.h5", overwrite=True, include_optimizer=True)

# with open('batch_history2.pkl', mode='wb') as f:
#     pickle.dump(history.history, f, protocol=pickle.HIGHEST_PROTOCOL)
# # plt.plot(history.history['loss'])
# # plt.xlabel('Epochs')
# # plt.ylabel('Adam') 
...
# # saving
# model_3.save(filepath="batch_model3.h5", overwrite=True, include_optimizer=True)

# with open('batch_history3.pkl', mode='wb') as f:
#     pickle.dump(history.history, f, protocol=pickle.HIGHEST_PROTOCOL)
# # plt.plot(history.history['loss'])
# # plt.xlabel('Epochs')
# # plt.ylabel('Adam') 
...
# optimizer
sgd = keras.optimizers.SGD(learning_rate=0.001, momentum=0.0,name='batch')
earlystopping = keras.callbacks.EarlyStopping(monitor='loss',
                                              min_delta=1e-4,
                                              patience=1,
                                              verbose=1)
...
# saving
model.save(filepath="batch_model4.h5", overwrite=True, include_optimizer=True)

with open('batch_history4.pkl', mode='wb') as f:
    pickle.dump(history.history, f, protocol=pickle.HIGHEST_PROTOCOL)
plt.plot(history.history['loss'])
plt.xlabel('Epochs')
plt.ylabel('Batch')
