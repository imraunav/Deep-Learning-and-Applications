...
def make_rnn():
    ...
    model = Sequential()
    model.add(layers.Input(shape=(200, 2), dtype=np.float64))
    model.add(layers.Masking(mask_value=100_0000.0)) #, input_shape=(seq_len, features)
...
def main():
    ...
    mask_val = 100_0000.0   # number unlikely to appear in normalized data
